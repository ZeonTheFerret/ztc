...
static Stmt *parse_if(Parser *p) {
    ...
    *s++ = '\0';
    char *var = xstrdup(var_start);  
    while (*s && isspace((unsigned char)*s)) s++;

    
    bool is_is;
    if (strncasecmp(s, "is not", 6) == 0) {
        is_is = false;
        s += 6;
...
static int transpile_to_c(ExecCtx *ctx, const char *out_path) {
    ...
    fprintf(stderr, "ztc: produced %s from %s\n", out_path, cpath);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "+f:e:lFvVh")) != -1) {
        switch (opt) {
